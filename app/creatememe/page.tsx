...
const CreateMeme = ({searchParams}: {searchParams: {id: string; url: string}}) => {
    ...
    const createMeme = async(event: React.FormEvent<HTMLFormElement>)=>{
        ...
        const data = await fetch(`https://api.imgflip.com/caption_image?template_id=${searchParams.id}&username=ayaanalam01&password=ayaan_786&text0=${text1.current?.value}&text1=${text2.current?.value}` , {
            method: 'POST'
        })
